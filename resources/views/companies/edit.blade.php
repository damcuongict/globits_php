        ...
        <div class="card">
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('companies.update', $company->id) }}">
                    ...
                    <div class="form-group">
                        <label for="departments">Sửa phòng ban</label>
                        @if ($departments)
                            @foreach($departments as $department)
                                <div class="input-group mb-3">
                                    <input type="text" class="form-control" name="department_{{ $department->id }}" value="{{ $department->name }}">                                  
                                </div>                           
                            @endforeach
                        @else
                            <p>Không có phòng ban nào được tìm thấy.</p>
                        @endif
                    </div>
